...
def frequency_stats(
    draws: List[Set[int]], n_bolas: int, recency_decay: float = None
) -> pd.DataFrame:
    ...
    df = df.sort_values(
        ["freq_ponderada", "freq", "dezena"], ascending=[False, False, True]
...
def fetch_last6m(jogo: str) -> pd.DataFrame:
    ...
    # pega último resultado global e a data limite (~6 meses)
...
# ---- CSS de estilo dos cards e bolas
st.markdown(
    """
<style>
.card{border:2px solid var(--c); border-radius:12px; padding:16px; margin:18px 0;}
.card-title{margin:0 0 14px; color:var(--c); font-weight:800; display:flex; gap:.6rem; align-items:center; font-size:22px;}
.card-blue{--c:#3498db;}
.card-purple{--c:#9b59b6;}
.card-green{--c:#27ae60;}
.card-red{--c:#e74c3c;}

.row-3 {display:flex; justify-content:space-between; gap:16px; font-weight:700; margin:8px 0 6px;}
.row-3 > span {font-size:18px;}
.row-3 > span.gold {color:#f1c40f;}

.balls{display:flex; flex-wrap:wrap; gap:8px; margin-top:10px;}
.ball{width:44px; height:44px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:700; color:#fff; box-shadow:0 2px 6px rgba(0,0,0,.25)}
...
# Estado para persistir resultados de palpites e aposta
if "palpites_df" not in st.session_state:
    st.session_state.palpites_df = None
if "aposta_aleatoria" not in st.session_state:
    st.session_state.aposta_aleatoria = None
...
st.markdown(
    f"<div class='row-3'>"
    f"<span>Concurso: {ultimo['concurso']}</span>"
    f"<span>Data: {ultimo['data']}</span>"
    f"<span class='gold'>Prêmio: R$ {valor_premio:,}</span>"
    f"</div>",
    unsafe_allow_html=True,
)
st.markdown("<h4>Dezenas sorteadas:</h4>", unsafe_allow_html=True)
st.markdown(
    "<div class='balls'>" + "".join(f"<div class='ball'>{d}</div>" for d in dezenas_ultimo) + "</div>",
    unsafe_allow_html=True,
)
st.markdown("</div>", unsafe_allow_html=True)  # fecha card

# =============================
# CARD: Palpites (quentes)
# =============================
...
n_palpites = st.number_input("Quantidade de palpites", 1, 200, 10, 1, key="n_palpites")
...
# Mostra última tabela gerada (dentro do card)
if st.session_state.palpites_df is not None:
...
# =============================
# CARD: Aposta aleatória (quentes + frios)
# =============================
...
if st.button("🎰 SORTEAR APOSTA ALEATÓRIA"):
    metade = n_escolhas // 2
    quentes = freq_df.head(max(1, n_bolas // 3))["dezena"].tolist()
    frios = freq_df.tail(max(1, n_bolas // 3))["dezena"].tolist()
    # evita repetição entre listas
    frios = [x for x in frios if x not in quentes]
    # garante tamanhos suficientes
    metade = min(metade, len(quentes))
    resto = n_escolhas - metade
    resto = min(resto, len(frios))
    escolha = sorted(random.sample(quentes, metade) + random.sample(frios, resto))
    st.session_state.aposta_aleatoria = escolha

if st.session_state.aposta_aleatoria:
    st.markdown(
        "<div class='balls'>" + "".join(f"<div class='ball'>{d}</div>" for d in st.session_state.aposta_aleatoria) + "</div>",
        unsafe_allow_html=True,
    )
st.markdown("</div>", unsafe_allow_html=True)  # fecha card
...
for _, row in ultimos5.iterrows():
    ...
    balls_html = "".join(f"<div class='ball'>{d}</div>" for d in dezenas)
    rows_html.append(
        f"<div class='last5-row'><b>Concurso {row['concurso']} ({row['data']})</b><br>"
        f"<div class='balls'>{balls_html}</div></div>"
    )
