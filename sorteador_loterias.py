...
def simulate_draws(freq_df: pd.DataFrame,
                   n_bolas: int,
                   n_escolhas: int,
                   n_sim: int,
                   power: float) -> List[Set[int]]:
    ...
    for _ in range(n_sim):
        chosen = set(np.random.choice(pool, size=n_escolhas, replace=False, p=w))
        sims.append(chosen)

    return sims

def evaluate_games(games: List[Set[int]],
                   simulated_draws: List[Set[int]]) -> pd.DataFrame:

    results = []

    for idx, game in enumerate(games, start=1):
        hits = [len(game & draw) for draw in simulated_draws]

        results.append({
            "JOGO": idx,
            "MEDIA_ACERTOS": np.mean(hits),
            "MAX_ACERTOS": np.max(hits),
            "PERC_>=4": np.mean(np.array(hits) >= 4),
            "PERC_>=5": np.mean(np.array(hits) >= 5),
            "PERC_>=6": np.mean(np.array(hits) >= 6),
        })

    df = pd.DataFrame(results)
    return df.sort_values("MEDIA_ACERTOS", ascending=False).reset_index(drop=True)

st.subheader("🧠 RANKING MONTE CARLO DOS JOGOS")

n_sim = st.slider("Quantidade de simulações", 500, 10000, 3000, step=500)

if st.button("▶️ EXECUTAR SIMULAÇÃO MONTE CARLO"):
    with st.spinner("Simulando milhares de sorteios possíveis..."):

        jogos_sets = [set(row[:n_escolhas]) for _, row in out_df.iterrows()]
        ...
        ranking = evaluate_games(jogos_sets, sims)
...
best = ranking.iloc[0]["JOGO"]
st.success(f"🏆 JOGO {int(best)} apresentou o melhor desempenho médio nas simulações.")
